...
public class SearchDBActivity extends ListActivity {	
	public static SearchDB SEARCHDB;
	
	public static void initializeSearchLogic(Context context) throws Exception {
		initializeSearchDBLogic(context, R.raw.sonnets);
	}
	
	public static void initializeSearchDBLogic(Context context, int resourceId) throws Exception {
		if(SEARCHDB == null) {
			SEARCHDB = new SearchDB(context, resourceId);
		}
	}
    ...
    private void loadView(Sonnet son) {
    	...
    	if(SEARCHDB == null) {
        	...
        	if (Intent.ACTION_SEARCH.equals(intent.getAction())) {
        		...
        		if(data != null) {
            		query = data.toString().substring(SearchDBProvider.CONTENT_URI.toString().length()+1).trim();
            		try { 
            			loadView(SEARCHDB.getSonnet(Integer.parseInt(query)));
            		} catch (Exception e) { 
            			e.printStackTrace(); 
            		}
            		return;
    ...
    private final Handler loadViewHandler = new Handler() { 
    	public void handleMessage(Message msg) { 
    		if(msg != null) {
    			loadView(SEARCHDB.getSonnet(msg.what));
    		}
    	}
    };
    ...
    private void search(String query) {
    	if(query != null) {
	        final SonnetFragment[] sfrags = SEARCHDB.search(query);
